...
namespace Automation
{
    class SiteDriver
    {
        ...
        public static void MoveToElement(IList<IWebElement> list)
        {

            Actions hover = new Actions(_webDriver);
            if (list.Count > 1)
            {
                for (int i = 1; i < list.Count; i++)
                {
                    //hover.ClickAndHold(list[i].Text).Perform();
                    hover.MoveToElement(list[i]).Perform();
